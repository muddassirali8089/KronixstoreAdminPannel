...
export const _orders = [...Array(24)].map((_, index) => ({
  id: (<Link to={`/order-details/${order_id(index)}`}>{order_id(index)}</Link>),
  name: _fullName(index),
  dateTime: formattedDate,
  isVerified: _boolean(index),
  avatarUrl: `/assets/images/avatar/avatar-${index + 1}.webp`,
  status: (<Label color="error" variant="inverted">
    +3
  </Label>),
  role:
    [
...
export const _products = [...Array(24)].map((_, index) => {
  ...
  return {
    ...
    description: _description(index),
    mrp: setIndex % 3 ? null : _price(index),
    colors:
...
export const _category = [...Array(24)].map((_, index) => {
  const setIndex = index + 1;
